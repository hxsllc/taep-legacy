...
if ($key1)
{
    ...
    {
        echo "<body><div class='sidesearchcolumn2'>Names Found</div>";
        ...
        while ($myrow = mysqli_fetch_array($result))
        {
            ...
            printf("<p class=\"footnote\"><a href=\"%s\" target=\"NameSearchForm\" onClick=\"top.setName('%s', '%s', -1)\"> %s </a>&nbsp;[%s]</p>\n", "SearchForm2.php", $myrow["code"], rawurlencode($name),$name,$myrow["name_count"]);
        }
    }
    ...
    if ( mysqli_num_rows($result) != 0)
    {
        ...
        if ($failed) {
                printf("<body><div class=\"sidesearchcolumn2\">Names Found</div>");
...
<?php
    ...
    printf("<body style='background:none;font-size:0.8em;'><center><h2>Name Search</h2>");

    printf("<form METHOD=POST onSubmit=\"return chkMaximum()\" ACTION=\"$PHP_SELF\" name=theform >");
    printf("<input type=text name=key1 value=\"\" size=18><br>");
    printf("<input type=submit value=\"Find Name\" ></center></form>");
	printf("<div class=\"footnote2\"><p>Search names with &amp; as \"Gold\" or \"Stock\" not \"Gold &amp; Stock\".</p><p>To find an individual, enter the last name but not the first name (i.e.,  \"Anthony\" will retrieve all the names containing \"Anthony\" but \"Anthony, Frank\" will not retrieve \"Anthony, F A\").</p><p>To find a company enter principal word in the name. (i.e.,  \"mechanic\" will find \"American Institute Mechanical Engineers,\" \"American Society of Mechanical Engineers,\" and \"Mechanical News\").</p></div>");	
}
